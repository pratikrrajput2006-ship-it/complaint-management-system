...
- Project requirements and complaint workflow planned
...
- `release()` implemented
- USER + ADMIN creation tested successfully
- `FOR UPDATE` concept implemented for sequence handling
- Automatic Admin ID generation implemented
- Admin IDs generated in the format `ADM001`, `ADM002`, `ADM003`, etc.
- Admin IDs are generated by the backend instead of being supplied by the client
- Admin sequence automatically increments using `next_number + 1`
- Deleted Admin IDs are not reused
- `ADM1000` and higher IDs are supported
...
- Admin login API
- Authentication using `bcrypt.compare()`
...
- Higher Authority account creation
